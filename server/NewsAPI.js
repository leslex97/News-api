...
app.post('/auth/login', cors(), (request, response) => {
	...
	if (!users[login]) {
		...
		if (users[login].password != password) {
			...
			loginStatus = true;
			loggedUser.loginStatus = loginStatus;
			loggedUser.name = users[login].name;
			loggedUser.surname = users[login].surname;
			response.json({logged: true, message: `User ${login} login success .`});
			
		}
	}
});

app.get('/auth/loginStatus', cors(), function (request, response){
	response.header("Access-Control-Allow-Origin", "*");
	if (loginStatus === true){
	response.status(200).send(loggedUser);}
		...
		loggedUser.surname = null;
		response.status(200).send(loggedUser);

}});

app.post('/auth/logout', cors(), function(request,response){
	response.header("Access-Control-Allow-Origin", "*");
	loginStatus = false;
	response.json({logged: false, message: `Logout Success`});
